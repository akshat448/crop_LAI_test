...
class MODISExporter:
    ...
    """
    def __init__(self, locations_filepath=Path('data/dataset.csv'),
    ...
    def export(self, folder_name, data_type, coordinate_system='EPSG:4326', scale=750,
        ...
        imgcoll = ee.ImageCollection(self.collection_id) \
            ...
            .filterDate('2002-12-31', '2018-1-1')
        ...
        region = ee.FeatureCollection('projects/ee-nikheel121/assets/IND_adm2')
...
#         region = region.map(state_to_int)
        ...
        for state_id, county_id in np.unique(self.locations[['State Name', 'Dist Name']].values.astype("<U22"), axis=0):
            if major_states_only:
                if str(state_id) not in MAJOR_STATES:
                    print(f'Skipping state id {str(state_id)}')
                    continue

            fname = '{}_{}'.format(str(state_id), str(county_id))
            ...
            file_region = region.filterMetadata('NAME_1', 'equals', str(state_id))
            file_region = region.filterMetadata('NAME_2', 'equals', str(county_id))
#             print(file_region)
#             break
            ...
            while True:
                try:
                    ...
                    print(f'Retrying State {str(state_id)}, County {str(county_id)}')
                    time.sleep(10)
                    continue
                break
    ...
    def export_all(self, export_limit=None, major_states_only=True, check_if_done=True,
        ...
        self.update_parameters(locations_filepath=Path('data/dataset.csv'),

...
class Engineer:
    ...
    def __init__(self, cleaned_data_path=Path('data/img_output'),
        ...
        yield_data = load(yield_data_filepath)[['Year', 'State Name', 'Dist Name', 'RICE YIELD (Kg per ha)']]
        yield_data.columns = ['Year', 'State', 'District', 'Value']
        county_data = pd.read_csv(county_data_filepath)[['Dist Name', 'State Name',
                                                         'Longitude', 'Latitude','Statec','Distc']]
        county_data.columns = ['District', 'State', 'Longitude', 'Latitude','Statec','Districtc']
        self.yield_data = yield_data.merge(county_data, how='left', on=['State', 'District'])
    ...
    def process(self, num_bands=9, generate='histogram', num_bins=32, max_bin_val=4999,
        ...
        for yield_data in self.yield_data.itertuples():
            ...
            county = yield_data.District
            state = yield_data.State
            countyc=yield_data.Districtc
            statec=yield_data.Statec

            filename = f'{year}_{str(state)}_{str(county)}.npy'
            if filename in self.processed_files:
                ...
                yields.append(int(yield_data.Value))
                ...
                state_county_info.append(np.array([int(statec), int(countyc)]))

                print(f'County: {str(county)}, State: {state}, Year: {year}, Output shape: {image.shape}')
